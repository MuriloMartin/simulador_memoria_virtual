...
bool validPageCheck (PageTable *page_table, int page_index, char operation,time_t current_time){
...
time_t oldest_access = page_table->frames[0].lastAcess;

for (int i = 1; i < page_table->num_frames; i++) {
    printf("last acess: %d oldest acess: %d\n", page_table->frames->lastAcess, oldest_access);
    if (page_table->frames[i].lastAcess < oldest_access) {
        oldest_access = page_table->frames[i].lastAcess;
        chosen_index = i;
    }
}
printf("Chosen Index %x\n", chosen_index);
return chosen_index;
}    
...
bool page_found = validPageCheck(page_table, page_index, operation, current_time);
...
if (!page_found) {
    ...
    page_table->frames[index_replace].lastAcess = current_time;
}
    }
}
